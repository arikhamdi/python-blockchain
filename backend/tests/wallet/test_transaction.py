...
def test_transaction_update():
    ...
    to_first_again_amout = 25
    transaction.update(sender_wallet, first_recipient, to_first_again_amout)

    assert transaction.output[first_recipient] == first_amount + \
        to_first_again_amout
    assert transaction.output[sender_wallet.address] == sender_wallet.balance - \
        first_amount - next_amount - to_first_again_amout
    assert Wallet.verify(
        transaction.input['public_key'],
        transaction.output,
        transaction.input['signature']
...
def test_invalid_reward_transaction_invalid_amout():
    miner_wallet = Wallet()
